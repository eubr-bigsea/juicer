...
class Workflow:
    ...
    def sort_tasks(self):
        ...
        tasks_position = {}
        count_position = 0
        for i in range(0, len(self.workflow['workflow']['tasks'])):
            tasks_position[self.workflow['workflow']['tasks'][i]['id']] = count_position
            count_position += 1
        # Then, performs topological sorting
        workflow_graph = self.builds_workflow_graph(self.workflow['workflow']['tasks'])
        ...
        for task_id in sorted_tasks_id:
            self.sorted_tasks.append(self.workflow['workflow']['tasks'][tasks_position[task_id]])
    ...
    def builds_workflow_graph(self, tasks):
        ...
        for task in tasks:
            ...
            for port in task['ports']:
                # There is just an origin for each port
                if port['direction'] == 'out':
                    ports_out[port['id']] = task['id']
                # One port can go to multiples nodes
                else:
                    if not ports_in.has_key(port['id']):
                        ports_in[port['id']] = []
                    ports_in[port['id']].append(task['id'])
        for port_in in ports_in:
            for node in ports_in[port_in]:
                workflow_graph.add_edge(ports_out[port_in], node)
