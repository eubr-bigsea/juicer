...
def test_marker(generated_chart):
    ...
    for marker in markers:
        assert (
            marker is not None
        ), "'marker' field not found for one or multiple classes"
...
def test_showlegend(generated_chart):
    ...
    ), "incorrect showlegend value for one or multiple classes "
...
def test_y(generated_chart, get_df):
    ...
    expected_ys = (
        get_df.groupby(["class", "petallength"])
        .agg(pl.avg("petallength").alias("avg_pl"))
        .collect()
    )
    # import pdb; pdb.set_trace()
    ys = [trace.get("y") for trace in data]
    expected_ys = [
        [1.5, 1.0, 1.4000000000000001, 1.9, 1.7, 1.5999999999999999, 1.1, 1.2, 1.3],
        [
            4.5,
            3.6,
            4.8,
...
def test_template(generated_chart):
    ...
    ), "'scatter' field not found in 'data' of 'template' "
