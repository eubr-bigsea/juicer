...
def test_coloraxis(generated_chart):
    ...
    assert (
        coloraxes == expected_coloraxes
    ), "Incorrect 'coloraxis' value for one or more series"
...
def test_hovertemplate(generated_chart):
    ...
    ), "Incorrect hovertemplate for one or more series"
...
def test_notched(generated_chart):
    ...
    assert (
        box_notched == expected_notched
    ), "Incorrect 'notched' value for 'box' type series"
...
def test_scalegroup(generated_chart):
    data, _ = generated_chart
    violin_scalegroup = [
        trace.get("scalegroup") for trace in data if trace.get("type") == "violin"
    ]
    expected_scalegroup = ["y"]
    assert (
        violin_scalegroup == expected_scalegroup
    ), "Incorrect 'scalegroup' value for 'violin' type series"
...
def test_xbingroup(generated_chart):
    data, _ = generated_chart
    histogram2d_xbingroup = [
        trace.get("xbingroup") for trace in data if trace.get("type") == "histogram2d"
    ]
    expected_xbingroup = ["x"]
    assert (
        histogram2d_xbingroup == expected_xbingroup
    ), "Incorrect 'xbingroup' value for 'histogram2d' type series"
...
def test_ybingroup(generated_chart):
    data, _ = generated_chart
    histogram2d_ybingroup = [
        trace.get("ybingroup") for trace in data if trace.get("type") == "histogram2d"
    ]
    expected_ybingroup = ["y"]
    assert (
        histogram2d_ybingroup == expected_ybingroup
    ), "Incorrect 'ybingroup' value for 'histogram2d' type series"
