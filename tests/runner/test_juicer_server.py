...
import mock
from juicer.runner.juicer_server import JuicerServer, StateControlRedis
...
def test_runner_read_start_queue_success():
    ...
    with mock.patch('redis.StrictRedis',
        ...
        with mock.patch('subprocess.Popen') as mocked_popen:
            server = JuicerServer(config=config)
            ...
            # Start of testing
            server.read_start_queue("/tmp", "faked_minions.py",
                                    mocked_redis_conn)
            ...
            assert mocked_popen.call_args_list[0][0][0] == [
                'nohup', sys.executable, 'faked_minions.py', '-j', job_id]
