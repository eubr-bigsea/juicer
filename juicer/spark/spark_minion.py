...
class SparkMinion(Minion):
    ...
    def __init__(self, redis_conn, workflow_id, app_id, config):
        ...
        if spark_home:
            ...
            self.log.info(_('SPARK_HOME set to %s'), spark_home)
        else:
            self.log.warn(_('SPARK_HOME environment variable is not defined'))
    ...
    def _emit_event(self, room, namespace):
        def emit_event(name, message, status, identifier, **kwargs):
            self.log.debug(_('Emit %s %s %s %s'), name, message, status,
                           identifier)
            ...
            if name == 'update task':
                if status == 'RUNNING':
                    listener_util.post_event_to_spark(
                        self.spark_session, listener_util.TASK_START, data)

                elif status == 'COMPLETED' or status == 'ERROR':
                    listener_util.post_event_to_spark(
                        self.spark_session, listener_util.TASK_END, data)

            elif name == 'update job':
                if status == 'RUNNING':
                    listener_util.post_event_to_spark(
                        self.spark_session, listener_util.JOB_START, data)

                elif status == 'COMPLETED' or status == 'ERROR':
                    listener_util.post_event_to_spark(
                        self.spark_session, listener_util.JOB_END, data)
    ...
    def _process_message_nb(self):
        ...
        if msg_type == juicer_protocol.EXECUTE:
            self.log.info(_('Execute message received'))
            ...
            self.job_future = self._execute_future(job_id, workflow,
                                                   app_configs)
            self.log.info(_('Execute message finished'))
        elif msg_type == juicer_protocol.DELIVER:
            self.log.info(_('Deliver message received'))
            ...
            if job_id:
                self.log.info(_('Terminate message received (job_id=%s)'),
                                job_id)
                self.cancel_job(job_id)
            else:
                self.log.info(_('Terminate message received (app=%s)'),
                              self.app_id)
                self.terminate()

        else:
            self.log.warn(_('Unknown message type %s'), msg_type)
    ...
    def _perform_execute(self, job_id, workflow, app_configs):
        ...
        try:
            ...
            self.log.debug(_('Objects in memory after loading module: %s'),
                           len(gc.get_objects()))
            ...
            import traceback
            tb = traceback.format_exception(*sys.exc_info())
            self.log.exception(_('Unhandled error'))
            self._emit_event(room=job_id, namespace='/stand')(
    ...
    def get_or_create_spark_session(self, loader, app_configs):
        ...
        if not self.is_spark_session_available():
            self.log.info(_("Creating a new Spark session"))
            ...
            for option, value in self.config['juicer'].get('spark', {}).items():
                if value is not None:
                    self.log.info(_('Setting spark configuration %s'), option)
        ...
        self.log.info(_("Minion is using '%s' as Spark master",
