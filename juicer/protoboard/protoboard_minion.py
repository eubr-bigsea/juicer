...
log = logging.getLogger('juicer.protoboard.protoboard_minion')

locales_path = os.path.join(os.path.dirname(__file__), '..', 'i18n', 'locales')


class ProtoboardMinion(Minion):
    """
    Controls the execution of Protoboard code in Lemonade Juicer.
    """
    ...
    def __init__(self, redis_conn, workflow_id, app_id, config, lang='en'):
        ...
        self.transpiler = ProtoboardTranspiler(config)
        ...
        self.protoboard_config = config['juicer'].get('protoboard', {})
    ...
    def _perform_execute(self, job_id, workflow, app_configs):
        ...
        try:
            ...
            # force the protoboard context creation
            self.get_or_create_protoboard_session(loader, app_configs, job_id)
            ...
            new_state = self.module.main(
                self.get_or_create_protoboard_session(loader,
                                                      app_configs,
                                                      job_id),
                self._state,
                self._emit_event(room=job_id, namespace='/stand'))
    ...
    def get_or_create_protoboard_session(self, loader, app_configs, job_id):
        """
        """
        pass
    ...
    def message_processed(self, msg_type, wid, job_id, workflow):
        msg_processed = {
            ...
            'job_id': job_id,
            'workflow': workflow,
            'type': ProtoboardMinion.MSG_PROCESSED,
            'msg_type': msg_type,
