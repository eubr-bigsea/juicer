...
class StateControlRedis:
    ...
    def pop_start_queue(self, block=True) -> str:
        result = self.pop_queue(self.START_QUEUE_NAME, block)
        return result if result is not None else ""

    def push_start_queue(self, data):
        self.push_queue(self.START_QUEUE_NAME, data)

    def pop_app_queue(self, app_id, workflow_id, block=True, timeout=0):
        return self.pop_queue(
            self.QUEUE_APP.format(app_id, workflow_id), block, timeout
        )

    def push_app_queue(self, app_id, workflow_id, data):
        self.push_queue(self.QUEUE_APP.format(app_id, workflow_id), data)

    def get_app_queue_size(self, app_id, workflow_id):
        key = self.QUEUE_APP.format(app_id, workflow_id)
    ...
    def get_minion_status(self, app_id, workflow_id):
        key = f'key_minion_app_{app_id}:{workflow_id}'
        result = self.redis_conn.get(key)
        return result if result else None

    def set_minion_status(self, app_id, workflow_id, status, ex=30, nx=True):
    ...
    def unset_minion_status(self, app_id, workflow_id):
        key = f'key_minion_app_{app_id}:{workflow_id}'
        return self.redis_conn.delete(key)

    def pop_app_output_queue(self, app_id, workflow_id, block=True):
        key = f'queue_output_app_{app_id}:{workflow_id}'
        if block:
    ...
    def push_app_output_queue(self, app_id, workflow_id, data):
        key = f'queue_output_app_{app_id}{workflow_id}'
        self.redis_conn.rpush(key, data)

    def get_app_output_queue_size(self, app_id, workflow_id):
