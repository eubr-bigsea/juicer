...
class VisualizationOperation(Operation):
    ...
    def __init__(self, parameters, named_inputs, named_outputs):
        ...
        if self.type not in self.CHART_MAP_TYPES:
            ...
            for x in self.x:
                ...
                if x.get("binning") == "QUANTILES":
                    ...
                    tmp = [0] + quantiles + [100]
                    x["labels"] = [f"{tmp[i]}-{tmp[i+1]}%" for i in range(len(tmp) - 1)]
        ...
        self.transpiler_utils.add_import("from plotly.subplots import make_subplots")
        ...
        if self.type not in self.CHART_MAP_TYPES:
            ...
            self.shapes = [
                (inx, y.get("shape")) for inx, y in enumerate(self.y) if y.get("shape")
            ]
            # self.custom_shapes =
    ...
    def generate_code(self) -> str:
        template_path = os.path.join(
            os.path.dirname(__file__), "..", "templates", "visualization_polars.tmpl"
        )
