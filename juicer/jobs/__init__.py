...
from pyspark.sql.types import (StructType, StructField, IntegerType, DateType,
                               StringType, FloatType, BinaryType, DoubleType,
...
def get_data_source(payload, source_id: int):
    if not source_id:
    ...
    valid_user = (ds.get('user_id') == payload.get('user', {}).get('id') or
                  'ADMINISTRATOR' in payload.get('user', {}).get(
        'permissions', []))
    if not valid_user:
    ...
    if not ds.get('enabled'):
        raise ValueError(
            gettext('Data source is not enabled and cannot be converted.'))
...
def convert_data_source(payload):
    try:
        ...
        if ds.get('format') != 'CSV':
            raise ValueError(
                gettext('Only CSV files can be converted to PARQUET'))

        if ds.get('storage', {}).get('type') != 'HDFS':
            raise ValueError(
                gettext('Only CSV files stored in HDFS can be converted'))
