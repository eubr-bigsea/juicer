...
RUN curl -Lo $JUICER_HOME/jars/iceberg-spark-runtime-3.4_2.12-1.3.1.jar https://repo1.maven.org/maven2/org/apache/iceberg/iceberg-spark-runtime-3.4_2.12/1.3.1/iceberg-spark-runtime-3.4_2.12-1.3.1.jar
...
RUN echo "export SPARK_DIST_CLASSPATH=$(hadoop classpath --glob):/usr/local/juicer/jars/spark-lof_2.11-1.0.jar:/usr/local/juicer/jars/lemonade-spark-ext-1.0.jar:/usr/local/juicer/jars/iceberg-spark-runtime-3.4_2.12-1.3.1.jar:/usr/local/juicer/jars/postgresql-42.7.3.jar" >> /etc/profile.d/juicer.sh && \
