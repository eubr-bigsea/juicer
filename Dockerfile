...
COPY hdp-spark2.tar.gz $JUICER_HOME
ENV HDP_VERSION 3.1.5.0-152
ENV SPARK_HOME /opt/spark2

RUN tar -xf ${JUICER_HOME}/hdp-spark2.tar.gz -C /opt/ && \
...
RUN pip3 install -U pip wheel && \
    python -m pip install -r $JUICER_HOME/requirements.txt --no-cache-dir
...
RUN pybabel compile -d $JUICER_HOME/juicer/i18n/locales && \
    ...
    echo "export CLASSPATH=$(hadoop classpath --glob):/usr/local/juicer/jars/spark-lof_2.11-1.0.jar:/usr/local/juicer/jars/lemonade-spark-ext-1.0.jar" >> /etc/profile.d/juicer.sh && \
    ...
    rm ${JUICER_HOME}/hdp-spark2.tar.gz
