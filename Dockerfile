...
RUN tar -xf hdp-spark2.tar.gz -C /opt/ && rm hdp-spark2.tar.gz && \
	mv /opt/spark2/conf/etc/spark2/3.1.5.0-152/0/* /opt/spark2/conf/ && \
        mkdir -p /opt/spark2/jars/hwc && \
	mv /opt/spark2/jars/hive-warehouse-connector-assembly-1.0.0.3.1.5.0-152.jar /opt/spark2/jars/hwc/ && \
        mv /opt/spark2/jars/pyspark_hwc-1.0.0.3.1.5.0-152.zip /opt/spark2/jars/hwc/ && \
	sed -i 's/spark\.eventLog\.enabled true/spark\.eventLog\.enabled false/g' /opt/spark2/conf/spark-defaults.conf && \
	sed -i 's/spark\.driver\.extraLibraryPath \/usr\/hdp\/current\/spark2-client\/jars/spark\.driver\.extraLibraryPath  \/opt\/spark2\/jars\/hwc\/hive-warehouse-connector-assembly-1\.0\.0\.3\.1\.5\.0-152\.jar/' /opt/spark2/conf/spark-defaults.conf && \
